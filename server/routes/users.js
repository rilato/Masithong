    ...
    User.findOne({ email: req.body.email }, (err, user) => {
        ...
        user.comparePassword(req.body.password, (err, isMatch) => {
            ...
            user.generateToken((err, user) => {
                ...
                res.cookie("w_auth", user.token) // user.token(토큰)을 쿠키에 저장한다.
                    .status(200)
                    .json({
                        loginSuccess: true,
                        userId: user._id, // 로그인 성공시, loginSuccess: true를 보여주고, userID를 보여준다. 크롬 확장도구인 Redux_devTools의 State를 통해 확인 가능
                    });
            });
        });
    });
